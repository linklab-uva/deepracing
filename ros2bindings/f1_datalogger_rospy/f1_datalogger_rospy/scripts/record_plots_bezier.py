...
class PlotRecorder(Node):
    ...
    def __init__(self):
        ...
        self.sub = self.create_subscription(BCMessage, '/predicted_path', self.plot_callback, 1)
        ...
        image_folder_param = self.get_parameter_or("image_folder", Parameter("image_folder",value="plot_images_bezier") )
    ...
    def im_callback(self, msg : CompressedImage):
        ...
        if self.current_bc is None:
            self.get_logger().info('New bezier curve not yet received, skipping')
            return
        bc = self.current_bc.copy()
        bctorch = torch.from_numpy(bc).unsqueeze(0).float()
        M = bezier.bezierM(self.s,bctorch.shape[1]-1)
        points_out = torch.matmul(M,bctorch).squeeze()
        print(bctorch.shape)
        ...
        # self.ax.plot(x, x, '--', linewidth=5, color='firebrick')
            self.ax.plot(-self.x_scale*points_out[:,0], self.z_scale*points_out[:,1], color='blue', label='Predicted\n Bézier Curve')
            self.ax.scatter(-self.x_scale*bc[:,0], self.z_scale*bc[:,1], c='green', label='Bézier Curve\'s\n Control Points')
            legendloc='lower right'
        else:
            self.ax.set_xlim([-25,25])
           # self.ax.set_ylim([0,125])
            self.ax.plot(points_out[:,0], points_out[:,1], color='blue', label='Predicted\n Bézier Curve')
            self.ax.scatter(bc[:,0], bc[:,1], c='green', label='Bézier Curve\'s\n Control Points')
    ...
    def plot_callback(self, msg : BCMessage):
        self.current_bc = np.vstack((msg.control_points_x,msg.control_points_z)).transpose()
