...
int main(int argc, char** argv) {
	...
	desc.add_options()
		...
		("output_file,o", po::value<std::string>(&output_file)->default_value(std::string("out.txt")), "Output file to dump the steering angles to.")
		;
	po::variables_map vm;
	...
	std::shared_ptr<std::ofstream> file_out(new std::ofstream(output_file,std::fstream::out));
	int index = 1;
	...
	while (fs::exists(current_file)) {
		...
		std::cout << "Image : " << ground_truth->image_file << " has steering angle " << ground_truth->sample.m_steer <<std::endl;
