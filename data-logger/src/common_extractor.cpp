...
int main(int argc, char** argv) {
	...
	std::vector<double> udp_x, udp_steering, udp_throttle, udp_brake;
	udp_x.reserve(samples.size());
	udp_steering.reserve(samples.size());
	udp_throttle.reserve(samples.size());
	udp_brake.reserve(samples.size());
	for (auto it = samples.begin(); it != samples.end(); it++) {
		udp_x.push_back((double)(*it)->timestamp);
		udp_steering.push_back((double)(*it)->sample.m_steer);
		udp_throttle.push_back((double)(*it)->sample.m_throttle);
		udp_brake.push_back((double)(*it)->sample.m_brake);
	}
	...
	while (!image_names_in.eof()) {
		...
		alglib::real_1d_array alglib_udp_x, alglib_steering, alglib_throttle, alglib_brake;
		...
		alglib_steering.setcontent(interpolation_degree + 1, &udp_steering[start_index]);
		alglib_throttle.setcontent(interpolation_degree + 1, &udp_throttle[start_index]);
		alglib_brake.setcontent(interpolation_degree + 1, &udp_brake[start_index]);
		alglib::barycentricinterpolant steering_interpolant, throttle_interpolant, brake_interpolant;
		alglib::polynomialbuild(alglib_udp_x, alglib_steering, steering_interpolant);
		alglib::polynomialbuild(alglib_udp_x, alglib_throttle, throttle_interpolant);
		alglib::polynomialbuild(alglib_udp_x, alglib_brake, brake_interpolant);
		double steering_interp, throttle_interp, brake_interp;
		steering_interp = alglib::barycentriccalc(steering_interpolant, ts);
		throttle_interp = alglib::barycentriccalc(throttle_interpolant, ts);
		brake_interp = alglib::barycentriccalc(brake_interpolant, ts);
		file_out << image << "," << ts << "," << steering_interp << "," << throttle_interp << "," << brake_interp << std::endl;
	}
