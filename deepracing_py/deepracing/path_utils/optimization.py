...
class BrakingConstraint():
    def __init__(self, ds: np.ndarray, max_speed : float, factor=1.0):
        ...
        speeds : np.ndarray = np.flip(np.asarray([          125.0,  84.00,  46.00,  25.0,  0.00 ], dtype=ds.dtype))
        braking_limits : np.ndarray  = np.flip(np.asarray([ 40.25 , 40.25 , 35.00,  17.5,  15.5 ], dtype=ds.dtype)*factor)
...
class LinearAccelConstraint():
    def __init__(self, ds: np.ndarray, max_speed : float, factor=1.0):
        ...
        self.linearaccelmat=generate_linear_accel_mat(ds)
        self.buffer = np.zeros_like(self.ds)
        speeds = np.asarray([              0.00,  36.0,  88.8,  150.0])
        forward_accel_limits = np.asarray([17.5,  17.5,  0.00,  0.000])*factor
