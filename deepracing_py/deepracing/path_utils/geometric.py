...
def computeTangentsAndNormals(r, X, k = 3, ds = 1.0, ref = np.array([0.0,1.0,0.0], dtype=np.float64)):
    ...
    # raug = raug-raug[0]
    garbagespline : interp.BSpline = interp.make_interp_spline(r, X, k=k, bc_type="periodic")
    garbagesplineder : interp.BSpline = garbagespline.derivative()
    truedistances : np.ndarray = np.zeros_like(r)
    for i in range(1, truedistances.shape[0]):
        rsumsamp : np.ndarray = np.linspace(r[i-1], r[i], num=5)
    ...
    spline : interp.BSpline = interp.make_interp_spline(truedistances, X, k=k, bc_type="periodic")
