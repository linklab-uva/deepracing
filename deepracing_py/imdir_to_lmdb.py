...
def main(args):
    ...
    if(os.path.isdir(dbpath)):
        if args.override:
            ...
            print("Database folder " + dbpath+ " already exists. add --override if you wish to delete this and rebuild the dataset.")
            exit(0)
    ...
    if roi is not None:
        ...
        fr = float(roi[2])
        fc = float(roi[3])
        topleft = np.asarray([r,c], dtype=np.float64)
        cropfactors = np.asarray([fr, fc], dtype=np.float64)
        cropsize = np.round(cropfactors*(np.asarray(im.shape[0:2], dtype=np.float64)-topleft)).astype(np.int64)
    else:    
        ...
        cropsize = np.asarray([h,w], dtype=np.float64)
        topleft = np.asarray([r,c], dtype=np.float64)
        cropfactors = cropsize/(np.asarray(im.shape[0:2])-topleft)
        cropsize = cropsize.astype(np.int64)
        print("Selected ROI:")
        print((r,c,h,w))
        cv2.imshow(windowname, cv2.cvtColor(im[r:r+h, c:c+w], cv2.COLOR_RGB2BGR))
        cv2.waitKey(0)
        cv2.destroyWindow(windowname)
    topleft = topleft.astype(np.int64)
    print("Input Image Size: " + str(im.shape[0:2]))
    print("Top-Left: " + str(topleft))
    print("Crop Factors: " + str(cropfactors))
