...
def run_epoch(experiment, encoder, decoder, optimizer, dataloader, recon_loss, loss_weights, use_tqdm = False, plot=False):
    cum_loss = 0.0
    cum_param_loss = 0.0
...
def go():
  ...
  #  print("encoder:\n%s" % (str(encoder)))
    ...
    for dataset in dataset_config["datasets"]:
        ...
        current_dset = PD.ImageSequenceDataset(image_wrapper, sequence_length=sequence_length, keys=keys)
        ...
        with experiment.train():
            while i < num_epochs:
                ...
                if models_to_disk:
                    ...
                    decoderfile = decoderpostfix % (postfix-1)
                    optimizerfile = optimizerpostfix % (postfix-1)
                else:
                    encoderfile = "encoder.pt"
                    decoderfile = "decoder.pt"
                    optimizerfile = "optimizer.pt"
                print("Running Epoch Number %d" %(postfix))
                #dset.clearReaders()
                tick = time.time()
                run_epoch(experiment, encoder, decoder, optimizer, dataloader, recon_loss, loss_weights, use_tqdm=use_tqdm)
                tock = time.time()
                ...
                decoderout = os.path.join(output_directory,decoderfile)
                with open(decoderout,'wb') as f:
                    torch.save(decoder.state_dict(), f)
                with open(decoderout,'rb') as f:
                    experiment.log_asset(f,file_name=decoderpostfix %(postfix,) )
