...
from deepracing_models.nn_models.StateEstimationModels import  ProbabilisticExternalAgentCurvePredictor
import numpy as np
...
def run_epoch(experiment : comet_ml.Experiment, network : ProbabilisticExternalAgentCurvePredictor, optimizer : torch.optim.Optimizer, dataloader : data_utils.DataLoader, debug : bool = False, use_tqdm = False):
    num_samples=0.0
    if use_tqdm:
    ...
    for (i, datadict) in t:
        ...
        meancurves = torch.cat([valid_future_positions[:,0].unsqueeze(1), means], dim=1)
        if debug:
            pass
     ...
     #   print(scale_trils[0])
        ...
        distpos = torch.distributions.MultivariateNormal(pred_points, covariance_matrix=poscovar, validate_args=False)
        ...
        # logging information
        if((i%15)==0):
            experiment.log_metric("point_estimate_loss", point_estimate_loss.item())
            experiment.log_metric("NLL", NLL.item())
        if use_tqdm:
            curr_loss = loss.item()
            lossf += curr_loss
            t.set_postfix({"point_estimate_loss" : point_estimate_loss.item(), "NLL" : NLL.item()})
def go():
    ...
    print("net:\n%s" % (str(net)))
    net = net.double()
    with open("D:/f1_model_files/state_estimator/202966af935b4e9484f01860bed2bed4/epoch_100_params.pt", "rb") as f:
        net.load_state_dict(torch.load(f, map_location=next(net.parameters()).device), strict=False)
    ...
    experiment = comet_ml.Experiment(workspace="electric-turtle", project_name="probabilistic-state-estimation", auto_metric_logging=False)
