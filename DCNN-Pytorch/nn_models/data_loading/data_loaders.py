...
class F1ImageSequenceDataset(Dataset):
    def __init__(self, annotation_filepath, im_size, context_length = 25, sequence_length=25):
        ...
        self.len = len(self.annotations) - context_length - sequence_length
        self.images = None
        self.labels = None
        self.image_pickle_postfix = "_sequence_images.pt"
        self.label_pickle_postfix = "_sequence_labels.pt"
    ...
    def loadFiles(self):
        self.images = torch.zeros(len(self.annotations), 1, self.resize.size[0], self.resize.size[1], dtype = torch.float32)
        ...
        for idx in tqdm(range(len(self.annotations)),desc='Loading Data',leave=True):
            ...
            impil = PILImage.open( os.path.join( self.image_folder, fp ) )
            im = self.totensor( self.grayscale( self.resize( impil ) ) )
            self.images[idx] = im
    ...
    def __getitem__(self, index):
        if(self.images is not None and self.labels is not None):
            ...
            images = self.images[ images_start : images_end + 1]
