...
class BayesianFilter(torch.nn.Module):
    def __init__(self, speeds: torch.Tensor, braking_limits : torch.Tensor, num_points : int, bezier_order: int, \
    ...
    def forward(self, curves : torch.Tensor, deltaT : float):
        ...
        return torch.sum(probs[:,None,None]*curves.double(), dim=0, keepdim=True).type(self.bezierM.dtype)
