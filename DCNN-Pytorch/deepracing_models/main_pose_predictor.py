...
def go():
    ...
    if epochstart>1:
        net.load_state_dict(torch.load(os.path.join(output_directory,"epoch_%d_params.pt" %(epochstart)), map_location=next(net.parameters()).device))
        optimizer.load_state_dict(torch.load(os.path.join(output_directory,"epoch_%d_optimizer.pt" %(epochstart)), map_location=next(net.parameters()).device))
    else:
