...
class AdmiralNetPosePredictor(nn.Module):
    def __init__(self, cell='lstm', input_channels=3, sequence_length=10, context_length = 15, \
        ...
        if(self.cell=='lstm'):
           ...
           # self.linear_velocity_rnn = nn.LSTM(self.feature_length, hidden_dim, batch_first = True, num_layers = num_recurrent_layers)
          #  self.angular_velocity_rnn = nn.LSTM(self.feature_length, hidden_dim, batch_first = True, num_layers = num_recurrent_layers)
    ...
    def forward(self, x):
        ...
        y = self.relu(y)    
        #print(y.shape)
        x = x.view(-1, self.input_channels, self.imsize[0], self.imsize[1]) 
        x = self.conv1(x)
        x = self.Norm_1(x)
        x1 = self.relu(x)
        x = self.conv2(x1)
        ...
        x_position, new_hidden = self.position_rnn(x)#, (init_hidden,  init_cell) )
       ...
       # print(y.shape)
        ...
        x_position, final_hidden_position = self.position_rnn(  y , new_hidden )
        x_rotation, final_hidden_rotation = self.rotation_rnn(  y , new_hidden )
